...
#define file_out "outDogTiling.y"
...
void meanFilter(int windowRadius) {
  int x,y,i,j,xx,yy,xt,yt;
  int imgWidth=M;
  int imgHeight=N;

  for (x=0; x<imgWidth; x+=2) {
    for (y=0; y<imgHeight; y+=2) {
      for(xt=x; xt<min(x+2,M); xt++) {
      	for(yt=y; yt<min(y+2,N); yt++) {
      		
      	  newImg[yt][xt]=0;
          for (i=-windowRadius; i<=windowRadius; i++) {
            for (j=-windowRadius; j<=windowRadius; j++) {
              xx=min(max(xt+i,0),imgWidth-1);
              yy=min(max(yt+j,0),imgHeight-1);
              newImg[yt][xt]=newImg[yt][xt] + image_Y[yy][xx];
            }
    	  }
          newImg[yt][xt]=myround((double)newImg[yt][xt] / pow((double)(2*windowRadius+1),2));
		}
	  }	
    }
  }
