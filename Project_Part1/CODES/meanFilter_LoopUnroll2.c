...
void read() {
  ...
  for(i=0; i<N; i+=2) {
	...
	point=M%UNROLL; //UNROLL factor is 4
    for(j=0; j<point; j++) {
      image_Y[i+1][j]=fgetc(frame_c);
    }
    for(j=point; j<M; j+=UNROLL) {
       image_Y[i+1][j]=fgetc(frame_c);
       image_Y[i+1][j+1]=fgetc(frame_c);
       image_Y[i+1][j+2]=fgetc(frame_c);
       image_Y[i+1][j+3]=fgetc(frame_c);
	}
  }
  fclose(frame_c);
}
...
void write() {
  ...
  for(i=0; i<N; i+=2) {
	...
	point=M%UNROLL; //UNROLL factor is 4
    for(j=0; j<point; j++) {
      fputc(newImg[i+1][j],frame_yuv);
    }
    for(j=point; j<M; j+=UNROLL) {
      fputc(newImg[i+1][j],frame_yuv);
      fputc(newImg[i+1][j+1],frame_yuv);
      fputc(newImg[i+1][j+2],frame_yuv);
      fputc(newImg[i+1][j+3],frame_yuv);        
	}
  }
  fclose(frame_yuv);
}
