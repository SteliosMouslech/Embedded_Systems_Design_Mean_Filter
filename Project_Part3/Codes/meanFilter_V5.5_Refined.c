...
void meanFilter(int windowRadius) {
  ...
  point=M%UNROLL;
  point2=(M+(2*RADIUS)+1)%UNROLL;
  for (y=windowRadius; y<N+windowRadius; y++) {
    //Buffer changes
    if (y==windowRadius) {
      for (k=0; k<point2; k++) {
      ...
      for(k=point2; k<M+(2*RADIUS)+1; k+=UNROLL) {
        buff_pointer6[k]=integralImg[y+3][k];
        buff_pointer6[k+1]=integralImg[y+3][k+1];
        buff_pointer6[k+2]=integralImg[y+3][k+2];
        buff_pointer6[k+3]=integralImg[y+3][k+3];
      }      
    }
    ...
    for (x=windowRadius+point; x<M+windowRadius; x++) {
      sum=0;
      xx=x+1;
	    tlj=xx-windowRadius;
	    rbj=xx+windowRadius;
